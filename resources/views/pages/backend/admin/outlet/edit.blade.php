            ...
            <div class="col-sm-12 pt-4">
                <div class="card">
                    <div class="card-header">
                        <h5>Update Data Outlet</h5>
                    </div>
                    <form method="POST" action="{{ route('admin.outlet.update', $outletById->id) }}"
                                ...
                                <div class="form-group col-md-12 mb-2">
                                    ...
                                    <div class="input-group mb-2">
                                        <input id="name" type="text" class="form-control" 
                                               value="{{ old('nama', $outletById->nama ?? '') }}" name="nama" required>
                                    </div>
                                </div>
                            </div>
                            <div class="form-row">
                                        ...
                                        <select class="js-example-basic-single" name="id_user" required>
                                            ...
                                            @foreach ($items as $item)
                                                <option value="{{ $item->id }}" 
                                                    {{ old('id_user', $outletById->id_user ?? '') == $item->id ? 'selected' : '' }}>
                                                    {{ $item->name }}
                                                </option>
                                            @endforeach                                            
                                        </select>
